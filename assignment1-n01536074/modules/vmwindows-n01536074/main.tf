resource "azurerm_availability_set" "win_availability_set" {
...
resource "azurerm_windows_virtual_machine" "vm_windows" {
  ...
  depends_on = [
    azurerm_availability_set.win_availability_set
  ]
  os_disk {
    name                 = each.key
  ...
  source_image_reference {
    ...
    boot_diagnostics {
    storage_account_uri = var.st_acc.primary_blob_endpoint
  }
}
